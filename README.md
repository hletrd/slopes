...
* 영상 재생 재시도 버튼 추가
* 동시 스트림 수 제한
* 4분할 뷰 재생 시 다른 영상 일시정지되도록 수정
* 영상 재생 시 백그라운드에서 재생중인 다른 오디오가 끊기지 않게 수정
* 모바일에서 캡처 시 파일 다운로드가 아닌 공유로 연결되게 변경
* 4분할 뷰에 캡처 및 PiP 추가
* 4분할 뷰를 별개의 PWA로 분리
* 영상 플레이할 때 백그라운드에서 재생되던 미디어 끊김 방지
  * PiP 모드일 때는 정책상 안 되는 것 같음.

#### 12-02
* 휘닉스파크 몽블랑 추가

#### 11-23
* 엘리시안, 알펜시아, 에덴벨리 링크 자동 업데이트 기능 추가
* 오투리조트 카메라 4개 추가
* 라이트 모드 추가
* 4분할 뷰 추가

#### 11-12
* 재오픈 준비, 링크 자동 업데이트 액션 살림.
...
* [x] ~~TypeScript / Next.js로 포팅~~
  ...
  * 일단 Next를 쓰는 순간 용량이 꽤 커지기 때문에 페이지 용량이 (외부 리소스 및 라이브러리 제외) 500kB가 넘기 전까지는 보류 예정
